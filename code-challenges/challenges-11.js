...
function square(arr) {
    ...
    const map1 = arr.map(element => element * element);
...
function gradesAvg(arr) {
    let avg;
    let mapAvg = arr.map(element => {
        let sum = 0;
        let numOfgrades = element.gradsList.length;
        for (let i = 0; i < numOfgrades; i++) {
            sum += element.gradsList[i];
        }
        element.avg = sum / numOfgrades;
        return element;
    })
    return mapAvg;
    // write your code here
